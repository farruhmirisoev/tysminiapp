...
var _shared = await jitiImport("@intlify/shared");
var _oxcParser = await jitiImport("oxc-parser");
var _compilerSfc = await jitiImport("@vue/compiler-sfc");
var _oxcWalker = await jitiImport("oxc-walker");
var _promises = await jitiImport("node:fs/promises");
var _ufo = await jitiImport("ufo");
var _unpluginVueRouter = await jitiImport("unplugin-vue-router");
var _options2 = await jitiImport("unplugin-vue-router/options");
var _magicString = _interopRequireDefault(await jitiImport("magic-string"));
var _unplugin = await jitiImport("unplugin");
var _nodeUrl = await jitiImport("node:url");
var _mlly = await jitiImport("mlly");
var _oxcTransform = await jitiImport("oxc-transform");
var _pluginYaml = _interopRequireDefault(await jitiImport("@rollup/plugin-yaml"));
var _rollupPluginJson = _interopRequireDefault(await jitiImport("@miyaneee/rollup-plugin-json5"));
var _unpluginVueI18n = _interopRequireDefault(await jitiImport("@intlify/unplugin-vue-i18n"));
var _nuxtDefine = await jitiImport("nuxt-define");
var _ohash = await jitiImport("ohash");
var _knitwork = await jitiImport("knitwork");
var _promises2 = await jitiImport("fs/promises");function _interopRequireDefault(e) {return e && e.__esModule ? e : { default: e };}function _interopRequireWildcard(e, t) {if ("function" == typeof WeakMap) var r = new WeakMap(),n = new WeakMap();return (_interopRequireWildcard = function (e, t) {if (!t && e && e.__esModule) return e;var o,i,f = { __proto__: null, default: e };if (null === e || "object" != typeof e && "function" != typeof e) return f;if (o = t ? n : r) {if (o.has(e)) return o.get(e);o.set(e, f);}for (const t in e) "default" !== t && {}.hasOwnProperty.call(e, t) && ((i = (o = Object.defineProperty) && Object.getOwnPropertyDescriptor(e, t)) && (i.get || i.set) ? o(f, t, i) : f[t] = e[t]);return f;})(e, t);}
...
function filterLocales(ctx, nuxt) {
  ...
  const include = toArray(project?.bundle?.onlyLocales ?? []).filter(_shared.isString);
  if (!include.length) {
    return ctx.options.locales;
  }
  return ctx.options.locales.filter((x) => include.includes((0, _shared.isString)(x) ? x : x.code));
}
function resolveLocales(srcDir, locales) {
  const localesResolved = [];
  for (const locale of locales) {
    const resolved = (0, _shared.assign)({ meta: [] }, locale);
    delete resolved.file;
    delete resolved.files;
    for (const f of getLocaleFiles(locale)) {
      const path = (0, _pathe.resolve)(srcDir, f.path);
...
function getLocaleType(path) {
  ...
  const parsed = (0, _oxcParser.parseSync)(path, (0, _nodeFs.readFileSync)(path, "utf-8"));
...
async function resolveVueI18nConfigInfo(rootDir, configPath = "i18n.config") {
  const absolutePath = await (0, _kit.resolvePath)(configPath, { cwd: rootDir, extensions: EXECUTABLE_EXTENSIONS });
  if (!(0, _nodeFs.existsSync)(absolutePath)) return void 0;
  return {
    path: absolutePath,
    // absolute
...
const getLocaleFiles = (locale) => {
  return toArray(locale.file ?? locale.files).filter((x) => x != null).map((x) => (0, _shared.isString)(x) ? { path: x, cache: void 0 } : x);
};
function resolveRelativeLocales(locale, config) {
  return getLocaleFiles(locale).map((file) => ({
    path: (0, _pathe.resolve)(config.langDir, file.path),
    cache: file.cache
  }));
}
const mergeConfigLocales = (configs) => {
  ...
  for (const config of configs) {
    for (const locale of config.locales ?? []) {
      const current = (0, _shared.isString)(locale) ? { code: locale, language: locale } : (0, _shared.assign)({}, locale);
      const files = (0, _shared.isString)(locale) ? [] : resolveRelativeLocales(current, config);
      delete current.file;
      ...
      merged.set(current.code, (0, _shared.assign)({}, current, existing));
    }
  }
  return Array.from(merged.values());
};
function getHash(text) {
  return (0, _nodeCrypto.createHash)("sha256").update(text).digest("hex").substring(0, 8);
}
function getLayerI18n(configLayer) {
  const layerInlineOptions = (configLayer.config.modules || []).find(
    (mod) => (0, _shared.isArray)(mod) && (0, _shared.isString)(mod[0]) && [NUXT_I18N_MODULE_ID, `${NUXT_I18N_MODULE_ID}-edge`].includes(mod[0])
  )?.[1];
  if (configLayer.config.i18n) {
    return (0, _defu.defu)(configLayer.config.i18n, layerInlineOptions);
  }
...
const logger = (0, _kit.useLogger)("nuxt-i18n");
...
function checkLayerOptions(_options, nuxt) {
  ...
  for (const layer of layers) {
    ...
    const layerHint = `In ${configLocation} layer (\`${(0, _pathe.resolve)(project.config.rootDir, layer.configFile)}\`) -`;
    try {
      if (!layerI18n.langDir) continue;
      if ((0, _shared.isString)(layerI18n.langDir) && (0, _pathe.isAbsolute)(layerI18n.langDir)) {
        logger.warn(
...
function resolveI18nDir(layer, i18n, i18nDir = i18n.restructureDir ?? "i18n") {
  return (0, _pathe.resolve)(layer.config.rootDir, i18nDir);
}
async function applyLayerOptions(ctx, nuxt) {
  const configs = [];
  if ((0, _pathe.isAbsolute)(ctx.options.langDir || "")) {
    const config = { langDir: ctx.options.langDir, locales: [] };
    for (const locale of ctx.options.locales) {
      if ((0, _shared.isString)(locale) || !getLocaleFiles(locale)?.[0]?.path?.startsWith(config.langDir)) continue;
  ...
  for (const layer of nuxt.options._layers) {
    ...
    const langDir = (0, _pathe.resolve)(resolveI18nDir(layer, i18n), i18n.langDir ?? "locales");
    configs.push((0, _shared.assign)({}, i18n, { langDir, locales: i18n.locales }));
  }
  await nuxt.callHook(
    "i18n:registerModule",
...
async function resolveLayerVueI18nConfigInfo(options, nuxt = (0, _kit.useNuxt)()) {
  const resolvers = [];
  if (options.vueI18n && (0, _pathe.isAbsolute)(options.vueI18n)) {
    resolvers.push(resolveVueI18nConfigInfo((0, _pathe.parse)(options.vueI18n).dir, options.vueI18n));
  }
  for (const layer of nuxt.options._layers) {
...
function setupAlias({ userOptions: options }, nuxt) {
  ...
  const layerI18nDirs = nuxt.options._layers.map((l) => {
    ...
    return (0, _pathe.relative)(nuxt.options.buildDir, (0, _pathe.resolve)(resolveI18nDir(l, i18n), "**/*"));
  }).filter((x) => !!x);
  const moduleIds = Object.keys(modules);
  nuxt.options.typescript = (0, _defu.defu)(nuxt.options.typescript, {
    hoist: moduleIds,
    tsConfig: {
      include: layerI18nDirs
    }
  });
  nuxt.options.vite = (0, _defu.defu)(nuxt.options.vite, {
    optimizeDeps: {
      include: moduleIds
    }
  });
  ...
  ).map((x) => (0, _kit.directoryToURL)(x));
  for (const [moduleName, moduleFile] of Object.entries(modules)) {
    const module = (0, _kit.resolveModule)(moduleFile, { url: moduleDirs });
...
function getRoutePath(tokens) {
  return tokens.reduce((path, token) => {
    return path + (token.type === 2 /* optional */ ? `:${token.value}?` : token.type === 1 /* dynamic */ ? `:${token.value}()` : token.type === 3 /* catchall */ ? `:${token.value}(.*)*` : token.type === 4 /* group */ ? "" : (0, _ufo.encodePath)(token.value).replace(COLON_RE, "\\:"));
  }, "/");
}
...
function transform(id, input, options) {
  const oxcOptions = (0, _kit.tryUseNuxt)()?.options?.oxc?.transform?.options ?? {};
  return (0, _oxcTransform.transform)(id, input, { ...oxcOptions, ...options });
}
const pattern = [DEFINE_I18N_LOCALE_FN, DEFINE_I18N_CONFIG_FN].join("|");
const DEFINE_I18N_FN_RE = new RegExp(`\\b(${pattern})\\s*\\((.+)\\s*\\)`, "gms");
const ResourcePlugin = (options, ctx) => (0, _unplugin.createUnplugin)(() => {
  ...
  return {
    ...
    transform: {
      ...
      async handler(_code, id) {
        let code = _code;
        const staticImports = (0, _mlly.findStaticImports)(_code);
        for (const x of staticImports) {
          if (x.specifier.startsWith("\0")) continue;
          i18nPathSet.add(await (0, _kit.resolvePath)((0, _pathe.resolve)((0, _pathe.dirname)(id), x.specifier)));
        }
        if (/[cm]?ts$/.test(id)) {
...
export const i18nPathToPath = ${JSON.stringify(routeResources.i18nPathToPath, null, 2)};`;
  ...
  nuxt.hook(
    ...
    async (pages) => {
      ...
      for (const layer of nuxt.options._layers) {
        const pagesDir = (0, _pathe.resolve)(projectLayer.config.rootDir, layer.config.srcDir, layer.config.dir?.pages ?? "pages");
...
async function setupExperimentalTypedRoutes(userOptions, nuxt) {
  ...
  function createContext(pages) {
    ...
    context.scanPages = async function (watchers = false) {
      ...
      const dtsContent = await (0, _promises.readFile)(dtsFile, "utf-8");
      if (routeNamedMapTypeRE.test(dtsContent)) {
        await (0, _promises.writeFile)(dtsFile, dtsContent.replace(routeNamedMapTypeRE, "RouteNamedMapI18n"));
      }
    };
    return context;
  }
  (0, _kit.addTemplate)({
    filename: (0, _pathe.resolve)(nuxt.options.buildDir, "./types/i18n-generated-route-types.d.ts"),
    getContents: () => {
      return `// Generated by @nuxtjs/i18n
...
function analyzePagePath(pagePath, parents = 0) {
  const { dir, name } = (0, _pathe.parse)(pagePath);
  if (parents > 0 || dir !== "/") {
...
function getRouteOptions(route, localeCodes, ctx, defaultLocale, mode = "config") {
  ...
  for (const locale of resolvedOptions.locales) {
    if ((0, _shared.isString)(resolvedOptions.paths[locale])) {
      paths[locale] = resolveRoutePath(resolvedOptions.paths[locale]);
      continue;
    }
    if ((0, _shared.isString)(resolvedOptions.paths[defaultLocale])) {
...
function getI18nRouteConfig(absolutePath, vfs = {}) {
  let extract = void 0;
  try {
    const content = absolutePath in vfs ? vfs[absolutePath] : (0, _nodeFs.readFileSync)(absolutePath, "utf-8");
    if (!content.includes(DEFINE_I18N_ROUTE_FN)) return void 0;
    const { descriptor } = (0, _compilerSfc.parse)(content);
    const script = descriptor.scriptSetup || descriptor.script;
    if (!script) return void 0;
    const lang = typeof script.attrs.lang === "string" && /j|tsx/.test(script.attrs.lang) ? "tsx" : "ts";
    let code = script.content;
    (0, _oxcWalker.parseAndWalk)(script.content, absolutePath.replace(/\.\w+$/, "." + lang), (node) => {
      ...
      if (typeof script.attrs.lang === "string" && /tsx?/.test(script.attrs.lang)) {
        ...
        routeArgument = (0, _oxcParser.parseSync)("", transformed.code, { lang: "js" }).program.body[0].expression.arguments[0];
      }
...
const TransformMacroPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  return {
    ...
    transform: {
      ...
      handler(code) {
        const parsed = (0, _compilerSfc.parse)(code, { sourceMap: false });
        ...
        if (match?.[0]) {
          const scriptString = new _magicString.default(script.content);
...
const TransformI18nFunctionPlugin = (options) => (0, _unplugin.createUnplugin)(() => {
  return {
...
function collectMissingI18nFunctions(script, id) {
  const scopeTracker = new _oxcWalker.ScopeTracker({ preserveExitedScopes: true });
  const ast = (0, _oxcWalker.parseAndWalk)(script, id, { scopeTracker });
  const missing = /* @__PURE__ */new Set();
  (0, _oxcWalker.walk)(ast.program, {
    scopeTracker,
    enter(node) {
...
const HeistPlugin = (options, ctx, nuxt = (0, _kit.useNuxt)()) => {
  ...
  return (0, _unplugin.createUnplugin)(() => ({
    ...
    transform: {
      filter: {
        id: [shared, (0, _pathe.relative)(nuxt.options.rootDir, shared)]
      },
      handler(code) {
        const s = new _magicString.default(code);
...
async function extendBundler(ctx, nuxt) {
  ...
  (0, _kit.addBuildPlugin)(resourcePlugin);
  ...
  (0, _kit.addBuildPlugin)({
    vite: () => _unpluginVueI18n.default.vite(vueI18nPluginOptions),
    // eslint-disable-next-line @typescript-eslint/no-unsafe-return
    webpack: () => _unpluginVueI18n.default.webpack(vueI18nPluginOptions)
  });
  (0, _kit.addBuildPlugin)(TransformMacroPlugin(pluginOptions));
  if (ctx.options.autoDeclare && nuxt.options.imports.autoImport !== false) {
    (0, _kit.addBuildPlugin)(TransformI18nFunctionPlugin(pluginOptions));
  }
  const defineConfig = getDefineConfig(ctx);
  await (0, _nuxtDefine.addDefinePlugin)(defineConfig);
}
function getDefineConfig({ options, fullStatic, deploymentHash }, server = false, nuxt = (0, _kit.useNuxt)()) {
...
export { localeDetector }` : `const localeDetector = undefined
  ...
  nuxt.hook("nitro:config", async (nitroConfig) => {
    nitroConfig.externals = (0, _defu.defu)(nitroConfig.externals ?? {}, { inline: [ctx.resolver.resolve("./runtime")] });
    ...
    if (localePathsByType.yaml.length > 0) {
      nitroConfig.rollupConfig.plugins.push((0, _pluginYaml.default)({ include: localePathsByType.yaml }));
    }
    if (localePathsByType.json5.length > 0) {
      nitroConfig.rollupConfig.plugins.push((0, _rollupPluginJson.default)({ include: localePathsByType.json5 }));
    }
  ...
  const h3UtilsExports = await (0, _mlly.resolveModuleExportNames)(UTILS_H3_PKG, { url: "file:///C:/My%20projects/miniapp_z/telegram-miniapp/node_modules/@nuxtjs/i18n/dist/module.mjs" });
  (0, _kit.addServerImports)(
  ...
  (0, _kit.addServerPlugin)(ctx.resolver.resolve("runtime/server/plugin"));
...
async function resolveLocaleDetectorPath(nuxt) {
  ...
  const localeDetectorPath = await (0, _kit.resolvePath)((0, _pathe.resolve)(i18nDir, i18nLayerConfig.experimental.localeDetector), {
    cwd: nuxt.options.rootDir,
    extensions: EXECUTABLE_EXTENSIONS
  });
  const exists = (0, _nodeFs.existsSync)(localeDetectorPath);
  if (!exists) {
...
const resolver = (0, _kit.createResolver)("file:///C:/My%20projects/miniapp_z/telegram-miniapp/node_modules/@nuxtjs/i18n/dist/module.mjs");
const distDir = (0, _pathe.dirname)((0, _nodeUrl.fileURLToPath)("file:///C:/My%20projects/miniapp_z/telegram-miniapp/node_modules/@nuxtjs/i18n/dist/module.mjs"));
const runtimeDir = (0, _nodeUrl.fileURLToPath)(new URL("./runtime", "file:///C:/My%20projects/miniapp_z/telegram-miniapp/node_modules/@nuxtjs/i18n/dist/module.mjs"));
function createContext(userOptions) {
  ...
  return {
    ...
    fullStatic: void 0,
    deploymentHash: (0, _ohash.hash)(Date.now()).slice(0, 8)
  };
}
...
async function resolveLocaleInfo(ctx, nuxt) {
  ...
  ctx.normalizedLocales = ctx.options.locales.map((x) => (0, _shared.isString)(x) ? { code: x, language: x } : x);
...
function generateTemplateNuxtI18nOptions(ctx, opts, nuxt = (0, _kit.useNuxt)()) {
  ...
  return `// @ts-nocheck
export const localeCodes =  ${(0, _knitwork.genArrayFromRaw)(ctx.localeCodes.map((x) => (0, _knitwork.genString)(x)))}
export const localeLoaders = ${(0, _knitwork.genObjectFromRaw)(localeLoaderEntries)}
export const vueI18nConfigs = ${(0, _knitwork.genArrayFromRaw)(opts.vueI18nConfigs.map((x) => x.importer))}
export const normalizedLocales = ${(0, _knitwork.genArrayFromRaw)(opts.normalizedLocales.map((x) => (0, _knitwork.genObjectFromValues)(x, "  ")))}
/** client **/
${codeHMR || ""}
/** client-end **/`;
}
...
function simplifyLocaleOptions(ctx, _nuxt) {
  ...
  const hasLocaleObjects = locales?.some((x) => !(0, _shared.isString)(x));
...
function generateLoaderOptions(ctx, nuxt) {
  ...
  for (const locale of ctx.localeInfo) {
    ...
    for (const meta of locale.meta) {
      if (!importMapper.has(meta.path)) {
        const key = (0, _knitwork.genString)(`locale_${(0, _knitwork.genSafeVariableName)((0, _pathe.basename)(meta.path))}_${meta.hash}`);
        importMapper.set(meta.path, {
          key,
          relative: (0, _pathe.relative)(nuxt.options.buildDir, meta.path),
          cache: meta.cache ?? true,
          load: (0, _knitwork.genDynamicImport)(asI18nVirtual(meta.hash), { comment: `webpackChunkName: ${key}` })
        });
      }
  ...
  for (let i = ctx.vueI18nConfigPaths.length - 1; i >= 0; i--) {
    ...
    const key = (0, _knitwork.genString)(`config_${(0, _knitwork.genSafeVariableName)((0, _pathe.basename)(config.path))}_${config.hash}`);
    vueI18nConfigs.push({
      importer: (0, _knitwork.genDynamicImport)(asI18nVirtual(config.hash), { comment: `webpackChunkName: ${key}` }),
      relative: (0, _pathe.relative)(nuxt.options.buildDir, config.path)
    });
  }
...
function generateI18nTypes(nuxt, ctx) {
  ...
  const resolvedLocaleType = (0, _shared.isString)(generatedLocales.at(0)) ? "Locale[]" : "LocaleObject[]";
...
import type { ComposerCustomProperties } from '${(0, _pathe.relative)(
    (0, _pathe.join)(nuxt.options.buildDir, "types"),
    (0, _pathe.resolve)(ctx.runtimeDir, "types.ts")
  )}'
import type { Strategies, Directions, LocaleObject } from '${(0, _pathe.relative)(
    (0, _pathe.join)(nuxt.options.buildDir, "types"),
    (0, _pathe.resolve)(ctx.distDir, "types.d.mts")
  )}'
...
function prepareRuntime(ctx, nuxt) {
  ...
  (0, _kit.addPlugin)(resolver.resolve("./runtime/plugins/i18n"));
  if (nuxt.options.dev || nuxt.options._prepare) {
    (0, _kit.addPlugin)(resolver.resolve("./runtime/plugins/dev"));
  }
  (0, _kit.addPlugin)(resolver.resolve("./runtime/plugins/preload"));
  (0, _kit.addPlugin)(resolver.resolve("./runtime/plugins/route-locale-detect"));
  (0, _kit.addPlugin)(resolver.resolve("./runtime/plugins/ssg-detect"));
  (0, _kit.addPlugin)(resolver.resolve("./runtime/plugins/switch-locale-path-ssr"));
      ...
      configureServer(server) {
        ...
        server.ws.on("i18n:options-complex-invalidation", () => {
          if (ctx.options.experimental.typedOptionsAndMessages) {
            (0, _kit.useNitro)().hooks.hookOnce("dev:reload", reloadClient);
            return;
          }
          reloadClient();
        });
...
function prepareRuntimeConfig(ctx, nuxt) {
  nuxt.options.runtimeConfig.public.i18n = (0, _defu.defu)(nuxt.options.runtimeConfig.public.i18n, {
...
function prepareAutoImports({ resolver, userOptions: options, runtimeDir }, nuxt = (0, _kit.useNuxt)()) {
  (0, _kit.addComponent)({
    name: "NuxtLinkLocale",
  ...
  (0, _kit.addImports)([
  { name: "useI18n", from: (0, _kit.resolveModule)(vueI18nPath) },
  ...[
  "useRouteBaseName",
  "useLocalePath",
  "useLocaleRoute",
...
function prepareBuildManifest({ localeInfo }, nuxt) {
  nuxt.hook("build:manifest", (manifest) => {
    const langFiles = localeInfo.flatMap((locale) => locale.meta.map((m) => m.path)).map((x) => (0, _pathe.relative)(nuxt.options.srcDir, x));
...
async function prepareTypeGeneration({ resolver, options }, nuxt) {
  ...
  (0, _kit.addServerPlugin)(resolver.resolve("runtime/server/type-generation"));
  ...
  await (0, _promises2.mkdir)((0, _pathe.dirname)(dtsFile), { recursive: true });
  nuxt.hook("prepare:types", ({ references }) => {
    references.push({ path: declarationFile });
  });
}
