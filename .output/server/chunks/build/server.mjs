import process from 'node:process';globalThis._importMeta_=globalThis._importMeta_||{url:"file:///_entry.js",env:process.env};import { getCurrentInstance, ref, computed, hasInjectionContext, inject, shallowRef, toRaw, reactive, effectScope, isRef, isReactive, getCurrentScope, onScopeDispose, watch, nextTick, toRefs, markRaw, defineComponent, createElementBlock, provide, cloneVNode, h, Suspense, Fragment, useSSRContext, createApp, createVNode, Text, shallowReactive, mergeProps, toRef, onErrorCaptured, onServerPrefetch, unref, resolveDynamicComponent, isReadonly, isShallow, defineAsyncComponent } from 'vue';
import { o as hasProtocol, q as isScriptProtocol, r as joinURL, w as withQuery, s as sanitizeStatusCode, t as getContext, v as parsePath, x as parseQuery, $ as $fetch$1, y as baseURL, z as createHooks, A as executeAsync, i as createError$1, B as toRouteMatcher, C as createRouter$1, D as defu, E as getRequestURL, F as getCookie, G as getRequestHeader, H as destr, I as klona, J as createDefu, K as isEqual, L as withoutTrailingSlash, M as setCookie, N as deleteCookie } from '../nitro/nitro.mjs';
...
const _routes = [
  {
    name: "index",
    path: "/",
    component: () => import('./index-GD-f_nrO.mjs')
  },
  {
    name: "signin",
    path: "/signin",
    meta: { "middleware": false },
    component: () => import('./signin-boPSEU3n.mjs')
  },
  {
    name: "signup",
    path: "/signup",
    meta: { "middleware": false },
    component: () => import('./signup-CyCqTwEz.mjs')
  },
  {
    name: "password-recover",
    path: "/password-recover",
    meta: { "middleware": false },
    component: () => import('./password-recover-BrUdTVFx.mjs')
  },
  {
    name: "activate-username",
    path: "/activate/:username()",
    meta: { "middleware": false },
    component: () => import('./_username_-euyYs4oa.mjs')
  }
];
  ...
  ru: [
    {
      ...
      load: () => import(
        './ru-D8LUhJbE.mjs'
        /* webpackChunkName: "locale_ru_46json_b95d54cc" */
      ),
      cache: true
    }
  ],
  uz: [
    {
      ...
      load: () => import(
        './uz-ypk79ui4.mjs'
        /* webpackChunkName: "locale_uz_46json_bf7be80f" */
      ),
      cache: true
    }
  ]
...
function createNuxtI18nContext(nuxt, vueI18n, defaultLocale) {
  ...
  const loadMessagesFromServer = async (locale) => {
    ...
    const messages = await $fetch(`${"/_i18n/umYW8WEv"}/${locale}/messages.json`, { headers });
...
const i18n_EI7LsD1KYQADczz5hrChviGQCdVM8yUkvFEZLJpmnvM = /* @__PURE__ */ defineNuxtPlugin({
  ...
  async setup(_nuxt) {
    ...
    prerenderRoutes(localeCodes.map((locale) => `${"/_i18n/umYW8WEv"}/${locale}/messages.json`));
...
var OsgoPeriodType = /* @__PURE__ */ ((OsgoPeriodType2) => {
  OsgoPeriodType2["ONE_YEAR"] = "ONE_YEAR";
  OsgoPeriodType2["SEASON"] = "SEASON";
  OsgoPeriodType2["TILL_REGISTRATION"] = "TILL_REGISTRATION";
  OsgoPeriodType2["FOREIGN"] = "FOREIGN";
  return OsgoPeriodType2;
})(OsgoPeriodType || {});
const COMPENSATION = 4e7;
const STORAGE_KEYS = {
...
const useApi = () => {
  ...
  class AjaxError {
    constructor(err) {
      this.raw = err;
      this.creation = /* @__PURE__ */ new Date();
    ...
    toString() {
      return this.raw?.data?.error?.message ?? this.raw?.data?.message ?? this.raw?.data?.error_description ?? this.raw?.data?.error ?? this.raw?.message ?? "Unexpected error";
    }
  }
    ...
    try {
      ...
      if (response && typeof response === "object") {
        if ("error" in response) {
          throw new Error(response.error?.message || "Driver verification failed");
        }
        if ("result" in response) {
          const result = response.result;
  ...
  const createOsgoApplication = async (params) => {
    try {
      const response = await invokeService(
        "OsgoService",
      ...
      if (response && typeof response === "object") {
        ...
        if ("error" in response) {
          const errorMsg = response.error?.message || response.error?.messageUz || "Failed to create application";
          throw new Error(errorMsg);
        }
        if ("result" in response) {
          const result = response.result;
      ...
      if (response && typeof response === "object") {
        if (("code" in response || "message" in response) && !("id" in response)) {
          const errorMsg = response.message || response.messageUz || "Failed to create application";
          throw new Error(errorMsg);
        }
      }
  ...
  const updateOsgoApplication = async (params) => {
    try {
      const response = await invokeService(
        "OsgoService",
  ...
  const sendPaymentLink = async (method, contractId, phone, amount) => {
    try {
      ...
      if (response && typeof response === "object" && "error" in response) {
        const errorMsg = response.error?.message || response.error?.messageUz || "Failed to send payment link";
        throw new Error(errorMsg);
      }
      return response;
    } catch (error) {
  ...
  const activateUser = async (login, smsCode) => {
    try {
  ...
  const recoveryCode = async (login) => {
    try {
  ...
  const saveRecoveryPassword = async (login, password, smsCode) => {
    try {
      const response = await invokeService(
        "UserService",
...
const useMetaStore = /* @__PURE__ */ defineStore("meta", () => {
  const meta = ref(null);
  ...
  const fetchMeta = async () => {
    ...
    try {
      ...
      console.log("[MetaStore] Version response:", versionResponse);
      console.log("[MetaStore] Version response type:", typeof versionResponse);
      if (versionResponse && typeof versionResponse === "object" && "data" in versionResponse) {
        console.log("[MetaStore] Wrapped response, extracting .data");
        versionData = Number(versionResponse.data);
      } else {
      ...
      if (versionData !== meta.value?.version) {
        ...
        let metaData;
        if (dataResponse && typeof dataResponse === "object" && "data" in dataResponse) {
...
const useAuthStore = /* @__PURE__ */ defineStore("auth", () => {
  ...
  const sendRecoveryCodeError = ref(null);
  const recoveringPassword = ref(false);
  const recoverPasswordError = ref(null);
  const data = ref(null);
  const fetching = ref(false);
  ...
  const activate = async (login, smsCode) => {
  ...
  const sendRecoveryCode = async (login) => {
  ...
  const recoverPassword = async (login, password, smsCode) => {
...
const _sfc_main$1 = {
  ...
  setup(__props) {
    ...
    const stack = void 0;
    const _Error404 = defineAsyncComponent(() => import('./error-404-CWMam5pV.mjs'));
    const _Error = defineAsyncComponent(() => import('./error-500-DO_Y_uGX.mjs'));
...
export { COMPENSATION as C, STEPS as S, useMetaStore as a, useApi as b, useI18n as c, defineStore as d, entry$1 as default, useSwitchLocalePath as e, useTelegramWebApp as f, useAuthStore as g, useRouter as h, useNuxtApp as i, useRuntimeConfig as j, nuxtLinkDefaults as k, useRoute as l, defineNuxtRouteMiddleware as m, navigateTo as n, resolveRouteObject as r, useHead as u };
